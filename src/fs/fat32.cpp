...
bool FAT32::ReadFile(const char* path, uint8_t* buffer, uint32_t* size) {
    FAT32DirEntry entry;
    bool is_root = false;
    if (!ResolvePath(path, &entry, &is_root))
        return false;
    if (is_root)
        return false;
...
bool FAT32::ResolvePath(const char* path, FAT32DirEntry* out, bool* is_root) {
    ...
    while (path[i] != '\0') {
        ...
        if (path[i] == '\0') {
            if (out)
                fat_mem_copy((uint8_t*)out, (uint8_t*)&entry, sizeof(FAT32DirEntry));
            return true;
        }
...
bool FAT32::WriteFile(const char* path, const uint8_t* buffer, uint32_t size) {
    ...
    if (exists && ((existing.attributes & 0x10) != 0))
        return false;
    if (exists) {
...
bool FAT32::IsDirectory(const char* path) {
    FAT32DirEntry entry;
    bool is_root = false;
    if (!ResolvePath(path, &entry, &is_root))
        return false;
