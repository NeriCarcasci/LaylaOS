...
static uint32_t sys_isdir(const char* path) {
    if (!global_fat32) return 0;
    uint32_t addr = (uint32_t)path;
    if (addr < 0x00400000 || addr >= 0x00D00000) return 0;
    return global_fat32->IsDirectory(path) ? 1 : 0;
}
...
static uint32_t sys_writefile(const char* path, const uint8_t* data, uint32_t len) {
    if (!global_fat32) return (uint32_t)-1;
    if (!IsRing3Range((uint32_t)path, 1)) return (uint32_t)-1;
    if (!IsRing3Range((uint32_t)data, len)) return (uint32_t)-1;
    return global_fat32->WriteFile(path, data, len) ? 0 : (uint32_t)-1;
}

static uint32_t sys_readfile(const char* path, uint8_t* out, uint32_t max_len) {
    if (!global_fat32) return (uint32_t)-1;
    if (max_len > 65536) return (uint32_t)-1;
    if (!IsRing3Range((uint32_t)path, 1)) return (uint32_t)-1;
    if (!IsRing3Range((uint32_t)out, max_len)) return (uint32_t)-1;

    uint8_t* tmp = new uint8_t[65536];
    if (!tmp) return (uint32_t)-1;

    uint32_t size = 0;
    bool ok = global_fat32->ReadFile(path, tmp, &size);
    if (!ok) {
        delete[] tmp;
        return (uint32_t)-1;
    }
...
static uint32_t sys_set_raw_input(uint32_t enable) {
    Terminal* term = Terminal::GetActive();
    if (!term) return (uint32_t)-1;
    term->SetRawInputMode(enable != 0);
    return 0;
}
...
uint32_t SyscallDispatch(uint32_t esp) {
    ...
    case 40:
        ...
        break;
    case 41:
        frame->eax = sys_isdir((const char*)frame->ebx);
        break;
    case 50:
    ...
    case 91:
        ...
        break;
    case 92:
        frame->eax = sys_writefile((const char*)frame->ebx, (const uint8_t*)frame->ecx, frame->edx);
        break;
    case 93:
        frame->eax = sys_readfile((const char*)frame->ebx, (uint8_t*)frame->ecx, frame->edx);
        break;
    case 94:
        frame->eax = sys_set_raw_input(frame->ebx);
        break;
    case 88:
